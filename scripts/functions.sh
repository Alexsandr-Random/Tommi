...
for pkg in $required_pkgs; do
        ...
        if [ "" = "$pkg_ok" ]; then
          ...
          sudo apt -yq  install $pkg 1>/dev/null #2>&1
        fi
done
}
...
function mysql_autoremove(){
        service mysql stop
        apt-get -q -y -o Dpkg::Use-Pty=0 remove mariadb-server* 2>/dev/null
        echo -e "\n\033[33m Mysql Removed, config files and databases are stayed \033[0m"
}
function mysql_purge(){
        service mysql stop
        apt-get -q -y -o Dpkg::Use-Pty=0 remove mariadb-server*
        echo -e "\n Removing MySQL Dependensies............ "
        sys_clean
        echo -e "\n Removing MySQL Data............ "
        rm -rf  /var/lib/mysql /var/log/mysql* /var/log/upstart/mysql.log* /var/run/mysqld 2> /dev/null
...
apt-get -q -y -o Dpkg::Use-Pty=0 remove phpmyadmin* 2>/dev/null
echo -e "${YELLOW} PhpMyAdmin removed! ${NC}"
}                                                                                                 
##########################################################APACHE2 FUNCTIONS###############################################
function apache_install(){
apt-get -qq -y -o Dpkg::Use-Pty=0 install apache2 libapache2-mod-php
...
function apache_remove(){
        ...
        apt-get -q -y -o Dpkg::Use-Pty=0 remove apache2* 2>/dev/null
...
apt -qq -y install php php-mysql
php_v=$( php -v | grep "PHP" | grep -E -o '[0-9]\.[0-9]' )
echo -e "\n\033[32m PHP Installed, Current Version is: $php_v \033[0m"
}
php_v=$( php -v | grep "PHP" | grep -E -o '[0-9]\.[0-9]' )
function php_autoremove(){
    apt-get -q -y -o Dpkg::Use-Pty=0 remove php$php_v 2>/dev/null
    apt -qq -y autoremove && apt autoclean
    echo -e "\n\033[33m PHP$php_v Removed \033[0m"
}
function php_purge(){
    apt-get -q -y -o Dpkg::Use-Pty=0 remove php*
